        ...
        if self.filename in os.listdir():
            ...
            def body_types(f):
                ...
                for i in range(0, self.count_elements):
                    ...
                    elif element["Type"].lower() == 'private_ad':
                        body = f"Private Ad -------------------\n{element['text_data']}\nActual until: " \
                               f"{element['Date']} days\n------------------------------"
                        output_list.append(body)
                    elif element["Type"].lower() == 'useful_tips':
                        body = f'Useful Tips ------------------\n{element["text_data"]}\nAuthor: ' \
                               f'{element["Author"]}\n------------------------------'
                        output_list.append(body)
                    else:
            ...
            f = json.load(open(self.filename))
            check(self, f)
...
if __name__ == '__main__':
    if input('text_data from file? Yes/No ').capitalize() == 'No':
        new_publication_object = Publication(
            input('Choose what you want to publish (News, Private Ad, Useful Tips): ').upper().strip(),
            input('text_data: '))
        ...
        if input('JSON? Yes/No ').capitalize() == 'No':
            ...
            new_publication_object = Publication(
                input('Choose what you want to publish (News, Private Ad, Useful Tips): ').upper().strip(), text_data)
            new_publication_object.publishing()
        else:
