...
def percent_of_total_cnt(dictionary):
    ...
    for letter, count in dictionary.items():
        percentage_count = round(count * 100.0 / total_cnt, 1)
        new_dict[letter] = percentage_count

    return new_dict


percentage_count = percent_of_total_cnt(count_letters)
...
def create_csv2():
    ...
    pct = [*percentage_count.values()]

    d = {'letter_name': letter, 'count_all': count, 'count_uppercase': cnt_up, 'percentage_count': pct}
